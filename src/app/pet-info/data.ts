...
export async function fetchPets(delay = 3000) {
  console.log(`🐾 Starting to fetch pets... (will take ${delay}ms)`);

  // Simulate network delay
  await new Promise(resolve => setTimeout(resolve, delay));

  // if (Math.random() < 0.1) {
  //   throw new Error('Failed to fetch pets from database');
  // }
...
export async function fetchAppointments(petId = null, delay = 4000) {
  console.log(`📅 Starting to fetch appointments${petId ? ` for pet ${petId}` : ''}... (will take ${delay}ms)`);

  // Simulate network delay
  await new Promise(resolve => setTimeout(resolve, delay));

  // if (Math.random() < 0.15) {
  //   throw new Error('Appointments service temporarily unavailable');
  // }
